...
export function MobileBottomAd({ className = '' }: MobileAdProps) {
  ...
  // Function to measure container width accurately
  const measureContainerWidth = () => {
    if (adRef.current) {
      // Use getBoundingClientRect for more accurate measurement
      const width = adRef.current.getBoundingClientRect().width;
      console.log(`Mobile ad container width measured: ${width}px`);
  ...
  useEffect(() => {
    if (!isVisible || isLoaded || process.env.NODE_ENV !== 'production' || !isMeasured || adContainerWidth <= 0) {
      return;
    }
    ...
    const timer = setTimeout(() => {
      try {
        ...
        console.log(`Initializing mobile AdSense ad with width: ${adContainerWidth}px`);
  ...
  }, [isVisible, isLoaded, adContainerWidth, isMeasured]);
        ...
        >
          <ins
            ...
            style={{
              display: 'block',
              minHeight: '250px',
              width: adContainerWidth > 0 ? `${adContainerWidth}px` : '100%',
            }}
